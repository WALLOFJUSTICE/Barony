...
int initApp(char const * const title, int fullscreen)
{
	...
	if ( (glGenBuffers = (PFNGLGENBUFFERSPROC)SDL_GL_GetProcAddress("glGenBuffers")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glBindBuffer = (PFNGLBINDBUFFERPROC)SDL_GL_GetProcAddress("glBindBuffer")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glBufferData = (PFNGLBUFFERDATAPROC)SDL_GL_GetProcAddress("glBufferData")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glDeleteBuffers = (PFNGLDELETEBUFFERSPROC)SDL_GL_GetProcAddress("glDeleteBuffers")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glGenVertexArrays = (PFNGLGENVERTEXARRAYSPROC)SDL_GL_GetProcAddress("glGenVertexArrays")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glBindVertexArray = (PFNGLBINDVERTEXARRAYPROC)SDL_GL_GetProcAddress("glBindVertexArray")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glDeleteVertexArrays = (PFNGLDELETEVERTEXARRAYSPROC)SDL_GL_GetProcAddress("glDeleteVertexArrays")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glGenFramebuffers = (PFNGLGENFRAMEBUFFERSPROC)SDL_GL_GetProcAddress("glGenFramebuffers")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glDeleteFramebuffers = (PFNGLDELETEFRAMEBUFFERSPROC)SDL_GL_GetProcAddress("glDeleteFramebuffers")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glBindFramebuffer = (PFNGLBINDFRAMEBUFFERPROC)SDL_GL_GetProcAddress("glBindFramebuffer")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glFramebufferTexture2D = (PFNGLFRAMEBUFFERTEXTURE2DPROC)SDL_GL_GetProcAddress("glFramebufferTexture2D")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glDrawBuffers = (PFNGLDRAWBUFFERSPROC)SDL_GL_GetProcAddress("glDrawBuffers")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glBlendFuncSeparate = (PFNGLBLENDFUNCSEPARATEPROC)SDL_GL_GetProcAddress("glBlendFuncSeparate")) == NULL )
	{
		noextensions = true;
	}
/*
// Unused
	else if ( (glEnableVertexAttribArray = (PFNGLENABLEVERTEXATTRIBARRAYPROC)SDL_GL_GetProcAddress("glEnableVertexAttribArray")) == NULL )
	{
		noextensions = true;
	}
	else if ( (glVertexAttribPointer = (PFNGLVERTEXATTRIBPOINTERPROC)SDL_GL_GetProcAddress("glVertexAttribPointer")) == NULL )
	{
		noextensions = true;
	}
*/
...
#else
	    SDL_GL_SetAttribute( SDL_GL_CONTEXT_MAJOR_VERSION, 4/*3*/ ); //Why GL 3.0? using only fixed pipeline stuff here
