...
void humanMoveBodyparts(Entity* my, Stat* myStats, double dist)
{
	...
	/* dummy armor stuff
	my->z -= 2.0 + 0.5 * cos(PI / 4 + 2 * PI * (ticks % 100) / 100.0);
	my->flags[INVISIBLE] = true;
	...
	for (bodypart = 0, node = my->children.first; node != nullptr; node = node->next, bodypart++)
	{
		...
		//{
		//	//entity->z -= 0.5 * cos(PI / 2 + 2 * PI * (ticks % 100) / 100.0);
		//}
		...
		//{
		//	entity->x += (1.0 + sin(PI * (ticks % 100) / 100.0)) * cos(my->yaw + PI / 2);
		//	entity->y += (1.0 + sin(PI * (ticks % 100) / 100.0)) * sin(my->yaw + PI / 2);
		//	entity->z += 1.0 + 1.0 * cos(PI / 4 + 2 * PI * (ticks % 100) / 100.0);
		//}
