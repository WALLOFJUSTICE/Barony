...
void useItem(Item* item, const int player, Entity* usedBy, bool unequipForDropping, bool serverCheckUse)
{
	...
	if ( players[player]->isLocalPlayer() )
	{
			...
			&& players[player]->entity == usedBy )
		{
			...
			if ( tryEmptyBottle && local_rng.rand() % 100 < std::min(80, (40 + skillLVL * 5)) ) // 40 - 65% chance
			{
...
Sint32 Item::weaponGetAttack(const Stat* const wielder) const
{
	...
	else if ( itemTypeIsQuiver(type) )
	{
		return attack + items[type].hasAttribute("ATK") ? items[type].attributes["ATK"] : 0;
	}
	else
	{
		attack += items[type].hasAttribute("ATK") ? items[type].attributes["ATK"] : 0;
	}
	// old formula
...
Sint32 Item::armorGetAC(const Stat* const wielder) const
{
	...
	{
		if ( items[type].hasAttribute("AC") )
		{
			armor += items[type].attributes["AC"];
		}
	}
